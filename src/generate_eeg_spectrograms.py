...
def parse_markers(vmrk_path: Path) -> dict[int, int]:
    ...
    for line in marker_lines:
        ...
        if not re.fullmatch(r"\d+", description):
            continue
        stimulus_positions[int(description)] = int(position)
...
def save_png(array_2d: np.ndarray, output_path: Path, preview_dpi: int) -> None:
    ...
    fig = plt.figure(figsize=(2, 2), dpi=preview_dpi)
    ax = fig.add_axes([0, 0, 1, 1])
    ax.imshow(array_2d, aspect="auto", origin="lower", cmap="magma")
    ax.axis("off")
    fig.savefig(output_path, dpi=preview_dpi, bbox_inches="tight", pad_inches=0)
    plt.close(fig)
...
def process_subject(
    ...
    # Normalization is done after all raw/log spectrograms are written so each latency condition
    # can use the subject-wide max computed from its own 0/100/300 ms subset.
    normalization_queue: list[dict[str, object]] = []
    subject_raw_max_by_latency: dict[int, float] = defaultdict(float)
    ...
    for row in timing_df.itertuples(index=False):
        ...
        file_stem = (
            f"{subject_dir.name}_stim{int(row_series['stimulus']):02d}"
            f"_idx{int(row_series['interval_index']):04d}"
            f"_word_{sanitize_token(row_series['word'])}"
        )
        ...
        for extra_window_ms in config.extra_window_ms_options:
            # Folder layout: <word>/<latency_ms>/{raw,raw_normalized,log_raw,log_normalized}
            variant_dir = word_dir / str(extra_window_ms)
            raw_dir = variant_dir / "raw"
            log_raw_dir = variant_dir / "log_raw"
            normalized_dir = variant_dir / "raw_normalized"
            log_normalized_dir = variant_dir / "log_normalized"
            ensure_dir(raw_dir)
            ensure_dir(log_raw_dir)
            ...
            variant_stem = f"{file_stem}_plus{extra_window_ms}ms"
            ...
            log_stacked_spectrogram = np.log10(np.maximum(raw_stacked_spectrogram, 0.0) + 1e-12)
            current_raw_max = float(np.max(raw_stacked_spectrogram))
            current_log_raw_max = float(np.max(log_stacked_spectrogram))
            subject_raw_max_by_latency[extra_window_ms] = max(
                subject_raw_max_by_latency[extra_window_ms], current_raw_max
            )
            subject_log_raw_max_by_latency[extra_window_ms] = max(
                subject_log_raw_max_by_latency[extra_window_ms], current_log_raw_max
            )

            # Each latency condition stores four representations:
            # raw, log_raw, raw_normalized, and log_normalized.
            raw_npy_path = raw_dir / f"{variant_stem}_raw_amplitude.npy"
            log_npy_path = log_raw_dir / f"{variant_stem}_log_amplitude.npy"
            np.save(raw_npy_path, raw_stacked_spectrogram)
            np.save(log_npy_path, log_stacked_spectrogram.astype(np.float32))
            ...
            if config.save_png:
                raw_png_file = (
                    raw_dir
                    / f"{variant_stem}_raw_amplitude_preview_ch_{sanitize_token(preview_channel_name)}.png"
                )
                log_png_file = (
                    log_raw_dir
                    / f"{variant_stem}_log_amplitude_preview_ch_{sanitize_token(preview_channel_name)}.png"
                )
                save_png(raw_stacked_spectrogram[0], raw_png_file, config.preview_dpi)
                save_png(log_stacked_spectrogram[0], log_png_file, config.preview_dpi)
                raw_png_path = str(raw_png_file)
                log_png_path = str(log_png_file)
            ...
            metadata_row = {
                ...
                "spectrogram_npy_log_amplitude": str(log_npy_path),
                "spectrogram_png_raw_amplitude": raw_png_path,
                "spectrogram_png_log_amplitude": log_png_path,
                "latency_raw_max_amplitude": "",
                "latency_log_raw_max_amplitude": "",
                "subject_raw_max_amplitude": "",
                "subject_log_raw_max_amplitude": "",
                "spectrogram_npy_normalized": "",
                "spectrogram_png_normalized": "",
    ...
    for item in normalization_queue:
        ...
        # Normalize with the max computed from the same latency bucket only.
        normalization_denominator = (
            subject_raw_max_by_latency[extra_window_ms]
            if subject_raw_max_by_latency[extra_window_ms] > 0.0
            else 1.0
        )
        ...
        # Read back the saved arrays so normalized outputs are guaranteed to match what was written to disk.
        raw_array = np.load(item["raw_npy_path"])
        log_array = np.load(item["log_npy_path"])
        normalized_array = (raw_array / normalization_denominator).astype(np.float32)
        log_normalized_array = (log_array / log_normalization_denominator).astype(np.float32)
        normalized_npy_path = item["normalized_dir"] / f"{item['variant_stem']}_raw_normalized.npy"
        log_normalized_npy_path = item["log_normalized_dir"] / f"{item['variant_stem']}_log_normalized.npy"
        np.save(normalized_npy_path, normalized_array)
        ...
        if config.save_png:
            normalized_png_file = (
                item["normalized_dir"]
                / f"{item['variant_stem']}_raw_normalized_preview_ch_{sanitize_token(item['preview_channel_name'])}.png"
            )
            log_normalized_png_file = (
                item["log_normalized_dir"]
                / f"{item['variant_stem']}_log_normalized_preview_ch_{sanitize_token(item['preview_channel_name'])}.png"
            )
            save_png(normalized_array[0], normalized_png_file, config.preview_dpi)
            save_png(log_normalized_array[0], log_normalized_png_file, config.preview_dpi)
            normalized_png_path = str(normalized_png_file)
            log_normalized_png_path = str(log_normalized_png_file)

        metadata_row["latency_raw_max_amplitude"] = float(subject_raw_max_by_latency[extra_window_ms])
        metadata_row["latency_log_raw_max_amplitude"] = float(
            subject_log_raw_max_by_latency[extra_window_ms]
        )
        metadata_row["subject_raw_max_amplitude"] = float(max(subject_raw_max_by_latency.values(), default=0.0))
        metadata_row["subject_log_raw_max_amplitude"] = float(
            max(subject_log_raw_max_by_latency.values(), default=0.0)
        )
        metadata_row["spectrogram_npy_normalized"] = str(normalized_npy_path)
        metadata_row["spectrogram_png_normalized"] = normalized_png_path
    ...
    summary_info = {
        ...
        "subject_raw_max_amplitude": float(max(subject_raw_max_by_latency.values(), default=0.0)),
        "subject_log_raw_max_amplitude": float(
            max(subject_log_raw_max_by_latency.values(), default=0.0)
        ),
        "raw_max_amplitude_0ms": float(subject_raw_max_by_latency.get(0, 0.0)),
        "raw_max_amplitude_100ms": float(subject_raw_max_by_latency.get(100, 0.0)),
        "raw_max_amplitude_300ms": float(subject_raw_max_by_latency.get(300, 0.0)),
